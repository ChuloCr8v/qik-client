...
const DashboardQuickActions = () => {
    ...
    return (
        <div
            className={`border border-border rounded-lg grid overflow-hidden bg-white ${teamEnabled ? "grid-cols-3" : "grid-cols-2"}`}
        >
            {actions.map((s, i) => {
                const Icon = s.icon;
                return (
                    <Button
                        key={s.title}
                        onClick={s.onOk}
                        className={`hover:bg-gray-100! bg-white! h-10! md:px-5! border-0! border-l! overflow-hidden flex items-center justify-start! gap-2 transition-colors rounded-none! ${i === 0 && "border-l-0!"} ${s.bgColor}!`}
                    >
                        <div
