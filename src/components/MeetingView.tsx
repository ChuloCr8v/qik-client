...
import { exportAgendaToMarkdown, exportAgendaToPDF } from '../utils/meetingExport';
import ActiveMeetingOverlay from './ActiveMeetingOverlay';
import Layout from './Layout';
import AgendaList from './meeting/AgendaList';
import AiCoachPanel from './meeting/AiCoachPanel';
import AttendeesPanel from './meeting/AttendeesPanel';
...
export default function MeetingView({ meetingId, onBack, onDeleteMeeting, onNavigate }: MeetingViewProps) {
  const { user } = useAuth();
      ...
      <Layout user={user} hideFooter onNavigate={onNavigate}>
        <div className="flex min-h-[calc(100vh-56px)] items-center justify-center px-4">
          <div className="max-w-md rounded-3xl border border-border bg-white p-6 text-center shadow-sm">
            <h1 className="text-xl font-bold text-secondary">Meeting unavailable</h1>
            <p className="mt-2 text-sm leading-relaxed text-muted">
              This meeting may not exist, or your signed-in email may not be on the invite list.
            </p>
            <button onClick={onBack} className="button-primary mt-5">
              Back to meetings
            </button>
          </div>
        </div>
  ...
  const completedTime = agenda.filter(item => item.completed).reduce((acc, item) => acc + item.duration, 0);
  ...
  return (
    <Layout user={user} hideFooter onNavigate={onNavigate}>
      <MeetingHeader
        ...
        onExportPDF={() => exportAgendaToPDF(meeting, agenda, realtime.participants)}
        onExportMarkdown={() => exportAgendaToMarkdown(meeting, agenda, realtime.participants)}
      ...
      <div className="relative mx-auto grid max-w-6xl items-start gap-6 px-4 py-4 sm:px-6 lg:grid-cols-4 lg:py-8">
        <div className="order-2 space-y-6 lg:sticky lg:top-24 lg:order-1 lg:col-span-1 lg:h-fit lg:self-start">
          ...
          <InviteesPanel invitees={meeting.invitees || []} onInvite={() => ui.setIsInviteModalOpen(true)} />
        </div>
        ...
        <AgendaList
          ...
          onToggleTemplates={() => ui.setIsTemplateListOpen(!ui.isTemplateListOpen)}
        ...
        <MeetingModals
          ...
          onCloseTemplate={() => ui.setSelectedTemplateForPreview(null)}
          onAddItem={actions.handleAddItem}
          onUpdateItem={(data) => actions.handleUpdateItem(ui.editingItem, data)}
          onApplyTemplate={async (startTime) => {
            if (!ui.selectedTemplateForPreview) return;
            await actions.applyTemplate(ui.selectedTemplateForPreview, startTime);
