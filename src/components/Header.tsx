...
import { Calendar, Bell, LogOut, Settings, User as UserIcon, ChevronDown, Menu, Check } from 'lucide-react';
import Dropdown, { DropdownItem } from './Dropdown';
import { formatDate } from '../lib/utils';
import { generateAnimeName, getAnimeAvatar } from '../lib/userUtils';
import { User } from '../types';
import { useAuth } from '../features/auth/AuthProvider';
import { useGetNotificationsQuery, useMarkNotificationAsReadMutation } from '../features/notifications/notificationsApi';
...
export default function Header({ user, onMenuClick, onNavigate, onGoogleSignIn }: HeaderProps) {
  const [showNotifications, setShowNotifications] = useState(false);
  const { signOut } = useAuth();
  const { data: notifications = [] } = useGetNotificationsQuery(undefined, { skip: !user, pollingInterval: 10000 });
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border bg-white/80 backdrop-blur-md">
      <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4 sm:px-6">
        <div className="flex items-center gap-2">
          <button 
            onClick={onMenuClick}
            className="p-2 -ml-2 text-muted hover:text-primary lg:hidden"
          >
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          {user ? (
            <>
                      ...
                      <div className="max-h-96 overflow-y-auto">
                        {notifications.length > 0 ? (
                            ...
                            >
                              <div className="flex gap-3">
                                ...
                                <div className="space-y-1">
                                  <p className="text-[11px] font-semibold text-secondary leading-tight">{n.title}</p>
                                  <p className="text-[10px] text-muted leading-relaxed">{n.message}</p>
                                  <p className="text-[9px] font-medium text-slate-400 uppercase tracking-wider">{n.createdAt ? formatDate(n.createdAt) : 'Just now'}</p>
                                </div>
                              </div>
                              {!n.read && (
                                <button 
                          ...
                          <div className="py-12 text-center">
                            ...
                            <p className="mt-2 text-[10px] text-muted uppercase tracking-widest">No notifications yet</p>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              <Dropdown
                trigger={
                  <button className="flex items-center gap-2 rounded-full border border-border p-1 pr-3 transition-hover hover:border-primary">
                    <img 
              ...
              >
                <div className="px-3 py-2 border-b border-border mb-1">
                  <p className="text-[10px] font-semibold text-muted uppercase tracking-widest">Signed in as</p>
                  <p className="text-xs font-semibold truncate text-secondary">{user.email || 'Guest Session'}</p>
                </div>
            ...
            <button 
              ...
              className="rounded-xl bg-primary px-4 py-2 text-xs font-semibold text-white shadow-sm hover:opacity-90 active:scale-95 transition-all"
            >
              Get Started
            </button>
          )}
