...
export default function Modal({ isOpen, onClose, title, children, footer, className, icon }: ModalProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
            ...
            >
              <div className="flex items-start justify-between px-6 py-5 border-b border-slate-50">
                ...
                <button 
                  onClick={onClose}
                  className="mt-0.5 rounded-xl p-1.5 text-muted hover:bg-slate-100 hover:text-primary transition-all shrink-0"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
              <div className="px-6 py-6 text-[12px] flex-1">
                {children}
              </div>
              {footer && (
