      ...
      footer={
        <div className="flex flex-row justify-end gap-3">
          <button
            type="button"
            onClick={onClose}
            className="text-sm font-semibold text-muted hover:bg-slate-100 rounded-xl transition-all"
          >
            Cancel
          </button>
          <button
            ...
            className={`flex items-center justify-center gap-2 text-sm font-semibold text-white   transition-all active:scale-[0.98] ${success ? 'bg-emerald-500 shadow-emerald-200' : 'bg-primary shadow-primary/20 hover:bg-primary/90'}`}
          >
            {isSending ? (
          ...
          <div className="rounded-2xl border border-border bg-slate-50 p-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {invitees.map(invitee => (
                <span key={invitee} className="rounded-full bg-white px-2 py-1 text-sm font-semibold text-secondary shadow-xs">
                  {invitee}
                </span>
              ))}
            </div>
