      ...
      <div className="flex-1 space-y-10 overflow-y-auto p-8">
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Agenda Pipeline</h3>
            <div className="rounded-full border border-white/5 bg-slate-900 px-2 py-0.5 text-xs font-bold text-slate-400">
              ...
              return (
                ...
                >
                  <div className="relative flex items-center gap-3 p-4">
                    <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-lg ${isCompleted
                      ? 'bg-emerald-500 text-slate-950'
                      : isActive
                        ? 'bg-amber-500 text-slate-950'
                        : 'bg-white/10 text-white/30'
                      }`}>
                      {isCompleted ? <CheckCircle2 className="h-3.5 w-3.5" /> : <span className="text-sm font-bold">{index + 1}</span>}
                    </div>

                    <div className="min-w-0 flex-1">
                      <p className={`truncate text-sm font-bold leading-none ${isActive || isCompleted ? 'text-white' : 'text-white/40'}`}>
                        {item.title}
                      </p>
                      <p className="mt-1 text-xs font-medium uppercase text-slate-500">{item.duration}m</p>
                    </div>
        ...
        <div className="space-y-6 border-t border-white/5 pt-10">
          <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Participants</h3>
          <div className="grid grid-cols-2 gap-3">
            {participants.map(participant => (
              <div key={participant.id} className="flex items-center gap-2 rounded-xl border border-white/5 bg-white/[0.03] p-2">
                ...
                />
                <span className="truncate text-sm font-bold text-white/60">{participant.displayName || generateAnimeName(participant.uid)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="border-t border-white/5 bg-slate-900/30 p-8">
        {nextItem ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-4 text-center">
            ...
            <p className="text-xs font-bold uppercase  text-emerald-500">Agenda Completed</p>
          </div>
        )}
      </div>
    </div>
