    ...
    <div className="space-y-3">
      ...
      <div className="space-y-3 rounded-xl border border-border bg-slate-50 p-4">
        ...
        <textarea
          ...
          className="min-h-[80px] w-full resize-none rounded-lg border border-border bg-white p-3 text-sm focus:border-primary focus:outline-none"
        />
        <button
          disabled={isGenerating || !aiContext.trim()}
          onClick={onGenerate}
          className="button-primary flex w-full items-center justify-center gap-2 py-1.5 text-sm"
        >
      ...
      <div className="space-y-2">
        <button
          ...
          onClick={onAnalyze}
          className="flex w-full items-center justify-center gap-2 rounded-lg border border-primary/20 bg-primary/5 py-2 text-sm font-bold text-primary transition-colors hover:bg-primary/10 disabled:opacity-50"
        >
        ...
        {analysisResult && (
          <div className="animate-in fade-in slide-in-from-top-2 rounded-lg border border-emerald-100 bg-emerald-50 p-3 text-left">
            <div className="mb-1.5 flex items-center justify-between">
              ...
              <button onClick={onDismissAnalysis} className="text-sm text-emerald-500 hover:text-emerald-700">Dismiss</button>
            </div>
