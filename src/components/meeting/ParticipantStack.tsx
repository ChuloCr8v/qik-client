...
export default function ParticipantStack({
    ...
    return (
        <div className="flex items-center gap-2">
                ...
                {visibleParticipants.map(participant => {
                    ...
                    return (
                        <div
                            key={participant.id}
                            title={name}
                            className="relative inline-block flex h-5 w-5 items-center justify-center overflow-hidden rounded-full bg-slate-100 text-xs font-semibold   ring-2 ring-white transition-transform hover:z-10 hover:scale-110"
                        >
                            <img
                                src={avatar}
                                alt={name}
            ...
            {remaining > 0 && (
                <span className="rounded-full border border-border bg-slate-50 px-1.5 py-0.5 text-xs font-semibold text-muted">
                    +{remaining}
                </span>
            )}
            {participants.length === 0 && (
                <span className="text-xs font-medium italic text-muted">
                    {meeting?.invitees?.length ? `${meeting.invitees.length} invited` : "No one yet"}
                </span>
            )}
        </div>
    );
