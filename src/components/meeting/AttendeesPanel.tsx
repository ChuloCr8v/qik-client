...
export default function AttendeesPanel({ participants }: AttendeesPanelProps) {
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Users className="h-3.5 w-3.5 text-secondary" />
          <h3 className="text-left text-[10px] font-semibold uppercase tracking-widest text-secondary">Attendees</h3>
        </div>
        <span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[8px] font-semibold text-muted">
          {participants.length}
        </span>
      </div>
      ...
      <div className="custom-scrollbar max-h-[300px] space-y-1.5 overflow-y-auto pr-1">
        {participants.length > 0 ? (
          participants.map(participant => {
            ...
            return (
              <div key={participant.id} className="group flex items-center gap-2 rounded-xl border border-slate-50 bg-white p-1.5 transition-all hover:border-primary/20 hover:shadow-sm">
          ...
          <p className="py-4 text-center text-[10px] italic text-muted">Waiting for others to join...</p>
        )}
      </div>
    </div>
  );
