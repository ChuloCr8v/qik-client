...
import { useDeleteAgendaItemMutation, useUpdateAgendaItemMutation } from '../../features/meetings/meetingsApi';
...
export default function AgendaItemRow({
  ...
  const toggleCompleted = () => {
    ...
    updateAgendaItem({ meetingId, itemId: item.id, data: { completed: !item.completed } });
  };
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex items-start justify-between gap-3">
            ...
            <button
              ...
              className={cn(
                'flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-semibold shadow-sm ring-1 ring-inset transition-all',
                isCurrent
            ...
            >
              ...
              <span className="whitespace-nowrap font-bold">
                {isCurrent
                  ? timerMode === 'left' ? `${formatTime(leftSeconds)} left` : `${formatTime(usedSeconds)} used`
                  : `${item.duration}m`}
              </span>
            </button>
          </div>
          ...
          <div className="mt-3 flex items-center justify-between border-t border-gray-100 pt-1">
            <div className="flex items-center gap-6">
              <button
                ...
                className={cn(
                  'mt-1.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 transition-all',
                  isCompleted
              ...
              <div
                ...
                className="mt-1 flex h-7 w-6 shrink-0 cursor-grab select-none items-center justify-center rounded bg-slate-50 text-muted transition-all active:cursor-grabbing group-hover:bg-primary/10 group-hover:text-primary"
              >
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => onEdit(item)}
                className="flex items-center gap-1.5 rounded-lg border border-border bg-white px-2.5 py-1 text-[10px] font-semibold text-slate-500 shadow-sm transition-colors hover:border-primary/20 hover:text-primary"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 rounded-lg border border-border bg-white px-2.5 py-1 text-[10px] font-semibold text-red-400 shadow-sm transition-colors hover:border-red-100 hover:text-red-600"
              >
