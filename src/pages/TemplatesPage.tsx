...
import TemplateGrid, { TemplateListItem } from '../components/templates/TemplateGrid';
...
import { useAddTemplateMutation, useDeleteTemplateMutation, useGetTemplatesQuery, useUpdateTemplateMutation } from '../features/templates/templatesApi';

export default function TemplatesPage() {
  const navigate = useNavigate();
  const { data: customTemplates = [], isLoading: loading, refetch } = useGetTemplatesQuery();
  ...
  const [editingTemplate, setEditingTemplate] = useState<(MeetingTemplate & { id: string }) | null>(null);
  ...
  const [formItems, setFormItems] = useState<Omit<AgendaItem, 'id' | 'order'>[]>([]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    const templateData = { name: formName, description: formDesc, items: formItems };

    try {
      if (editingTemplate) {
        await updateTemplate({ id: editingTemplate.id, data: templateData }).unwrap();
      } else {
  ...
  const handleApplyTemplate = async (template: MeetingTemplate, startTime?: string) => {
    try {
      const meeting = await createMeeting({ title: template.name, template, scheduledAt: startTime }).unwrap();
  ...
  const allTemplates: TemplateListItem[] = [
    ...MEETING_TEMPLATES.map(template => ({ ...template, id: `system-${template.name}`, isSystem: true })),
    ...customTemplates.map(template => ({ ...template, isSystem: false }))
  ];
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      <PageHeader
        ...
        description="Standardize your organizational workflows with agendas."
        action={
