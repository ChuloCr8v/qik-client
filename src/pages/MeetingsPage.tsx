...
import { FileText, Search, Filter, Trash2, Plus, Clock } from 'lucide-react';
...
export default function MeetingsPage({ meetings, onMeetingClick, onCreateMeeting, onDeleteMeeting }: MeetingsPageProps) {
      ...
      {filteredMeetings.length > 0 ? (
        <Table headers={['#', 'Title', 'Created', 'Status', 'Participants', '']}>
          {filteredMeetings.map((meeting, index) => (
            <TableRow key={meeting.id} onClick={() => onMeetingClick(meeting.id)}>
              <TableCell className="w-8 text-muted font-semibold text-xs">{index + 1}</TableCell>
              <TableCell className="flex items-center gap-3">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-500">
                  <FileText className="h-4 w-4" />
                </div>
                <div className="flex flex-col items-start gap-0.5">
                  <span className="text-xs font-semibold text-slate-900 leading-tight truncate">{meeting.title}</span>
                  {meeting.scheduledAt && (
        ...
        <div className="py-16 text-center border-2 border-dashed border-slate-100 rounded-3xl bg-slate-50/30">
          ...
          <div className="mt-4 space-y-1">
            <p className="text-sm font-semibold text-secondary">No meetings found</p>
