...
import { UserPlus, Shield, MoreHorizontal, Loader2, Users, Zap, CheckCircle2, Search, Filter } from 'lucide-react';
import { User } from '../types';
import TeamInviteModal from '../components/TeamInviteModal';
import { useListUsersQuery } from '../features/users/usersApi';

const DEMO_MEMBERS: User[] = [
  { uid: 'd1', displayName: 'John Doe', email: 'john@example.com', role: 'Owner', status: 'Active' },
  { uid: 'd2', displayName: 'Jane Smith', email: 'jane@example.com', role: 'Admin', status: 'Active' },
  { uid: 'd3', displayName: 'Mike Ross', email: 'mike@example.com', role: 'Member', status: 'Away' },
  { uid: 'd4', displayName: 'Harvey Specter', email: 'harvey@example.com', role: 'Member', status: 'Active' },
];
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-6">
          {[
            ...
            <div key={i} className="flex flex-col gap-2 rounded-2xl border border-border bg-white p-3 shadow-xs transition-all hover:shadow-md sm:p-5">
              <div className={`flex h-8 w-8 items-center justify-center rounded-xl sm:h-10 sm:w-10 ${stat.bg} ${stat.color}`}>
                <stat.icon className="h-4 w-4 sm:h-5 sm:w-5" />
              </div>
              <div>
                <p className="text-[9px] font-semibold uppercase tracking-wider text-muted sm:text-[10px]">{stat.label}</p>
                <p className="text-sm font-semibold text-secondary sm:text-2xl">{stat.value}</p>
              </div>
            </div>
          ))}
        </div>
